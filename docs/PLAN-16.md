...
- [x] T16.9.1 — README new section §"Connecting Claude Code to the Gateway":
...
- [x] T16.9.2 — README new section §"Commands vs MCP servers":
...
- [x] T16.9.3 — Modify `vscode/mcp-gateway-dashboard/src/slash-command-generator.ts:296` or wherever the skeleton template is built to prepend an explicit disclaimer:
  ```markdown
...
- [x] T16.9.4.a — **[REVIEW-16 L-04]** Before T16.9.4 swap, add CI workflow `dogfood-smoke.yml` that runs on PRs touching `.mcp.json` or `config.json`:
...
- [ ] T16.9.4 — **DEFERRED to operator follow-up** — Replace repo `.mcp.json` to dogfood through the gateway:
  ```json
  {
    "mcpServers": {
      "mcp-gateway": {
...
- [x] T16.9.5 — Author `docs/ADR-0005-claude-code-integration.md`:
...
- [x] T16.9.6 — CHANGELOG.md entry for v1.6.0:
...
- [x] T16.9.7 — ROADMAP.md update: mark Phase 16 (this /run slice: 16.5 + 16.7 + 16.8 + 16.9) complete; 16.4 + 16.6 tracked as in-flight on parallel windows; Phase 17 candidates noted in the existing v1.7.0+ backlog area.

- [x] T16.9.GATE: **PASSED 2026-04-22 for this /run scope (16.9 docs-only slice).** Scoped mocha `src/test/slash-command-generator.test.ts` → 31 pass 585 ms (includes new 3-line-header pinning assertions). `npm run compile` clean. ADR-0005 authored. CHANGELOG v1.6.0 entry added. README sections added. dogfood-smoke.yml CI workflow added. T16.9.4 (live `.mcp.json` swap) DEFERRED to operator — the swap needs the gateway running stably and is safest done outside the /run loop. PAL cross-validation deferred to `/finish` sweep because `docs/REVIEW-16.md` is still owned by the concurrent `feature-b8f2decf` pipeline.
