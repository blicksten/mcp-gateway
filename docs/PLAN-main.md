...
- [x] T12B.GATE — Tests + codereview + thinkdeep — zero MEDIUM+ — DONE 2026-04-18 after fix-round-1. PAL `codereview` (gpt-5.2-pro, security mode) on commit `7b1e52f` returned 1 CRITICAL + 3 HIGH + 5 MEDIUM. All 9 fixed in follow-up commit: (1) **CRITICAL** — `--json` with `Found=0` printed human text; now emits valid JSON for every exit path; (2) **HIGH** — dedicated `mcpGateway.mcpCtlPath` setting (stopped borrowing `daemonPath` which points at the wrong binary); (3) **HIGH** — `validateServerName` guards every KDBX-sourced server before SecretStorage writes so malformed names surface as `skipped` instead of polluting the index; (4) **HIGH** — `readPasswordStdin` operates on `[]byte` throughout with the original buffer zeroed on every exit path (password no longer transits an immutable Go `string`); (5) **MEDIUM** — `--password-stdin` capped via `io.LimitReader(n=4096)` with a clear error on overrun; (6) **MEDIUM** — `ExecException.code` is correctly type-checked for `number` vs `string` (ENOENT would have surfaced as spurious exit code); (7) **MEDIUM** — comment/behaviour consistency: `MCP_GATEWAY_AUTH_TOKEN` explicitly blanked in the child env; (8) **MEDIUM** — `applyImportedCredentials` uses per-entry try/catch so a single env-var failure no longer blocks the remaining env vars + headers; (9) **MEDIUM** — extension summary surfaces `skipped` + warnings as a warning toast instead of a misleading "Imported N" info. 3 new regression tests (Found=0 JSON, invalid-name skipped, per-entry partial) + 1 LOW deferred as non-blocking.
