...
- [x] **T4.1 (F-ARCH-3 — expanded from 6 to 9 items):** Manual E2E checklist at [docs/qa/server-rename-smoke.md](qa/server-rename-smoke.md):
...
- [x] GATE: tests + codereview + thinkdeep — zero errors (any finding at or above CLAUDE_GATE_MIN_BLOCKING_SEVERITY; default: any finding) — **PASSED 2026-05-12** PAL gpt-5.1-codex gate_mode=true security review on all Phase 1–3 changed files → `gate_verdict=PASS, 0 findings, none blocking`. T4.1 manual E2E checklist created at `docs/qa/server-rename-smoke.md` (9 items) — operator portion (T4.8); failures route to follow-up cycle.
