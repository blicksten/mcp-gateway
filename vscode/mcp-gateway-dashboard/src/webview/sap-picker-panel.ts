...
export class SapPickerPanel {
	...
	private async refresh(): Promise<void> {
		...
		if (!this.client.getSapPickerSnapshot) {
			await this.postError(
				'SAP Picker requires gateway daemon v1.8+ — upgrade the daemon and try again.',
			);
			return;
		}
		try {
			const snap = (await this.client.getSapPickerSnapshot()) as PickerSnapshot;
			this.latestSnapshot = snap;
			this.rows = initRowsFromSnapshot(snap);
			await this.postInit();
		} catch (err) {
			logger.error('sap-picker', 'snapshot fetch failed', err);
			await this.postError(`Failed to load SAP systems: ${errorMsg(err)}`);
		}
	}
	...
	private async refreshAfterApply(): Promise<void> {
		if (this.disposed) { return; }
		if (!this.client.getSapPickerSnapshot) { return; }
		try {
			const snap = (await this.client.getSapPickerSnapshot()) as PickerSnapshot;
			this.latestSnapshot = snap;
